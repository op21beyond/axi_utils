...
// Assumptions : WORD-only accesses by system contract.
...
module axi3_to_ahblite #(
    ...
    // AXI3 slave side (no ID/USER/QOS)
    ...
    reg [3:0]              wr_prot_fifo  [0:WR_CMD_DEPTH-1];
    reg [DATA_WIDTH-1:0]   wr_data_fifo  [0:WR_CMD_DEPTH-1];
    reg [WR_PTR_W-1:0]     wr_wr_ptr, wr_rd_ptr;
    reg [WR_PTR_W:0]       wr_count;

    reg [ADDR_WIDTH-1:0]   rd_addr_fifo  [0:RD_CMD_DEPTH-1];
    reg [3:0]              rd_prot_fifo  [0:RD_CMD_DEPTH-1];
    reg [RD_PTR_W-1:0]     rd_wr_ptr, rd_rd_ptr;
    reg [RD_PTR_W:0]       rd_count;

    reg [DATA_WIDTH-1:0]   rdata_fifo [0:RESP_DEPTH-1];
    ...
    reg [3:0]              wr_beats_left_q;
    reg [3:0]              wr_total_beats_q;
    reg [3:0]              wr_done_beats_q;
    reg [3:0]              wr_prot_q;

    reg                    rd_active;
    reg [ADDR_WIDTH-1:0]   rd_addr_q;
    reg [3:0]              rd_beats_left_q;
    reg [3:0]              rd_total_beats_q;
    reg [3:0]              rd_done_beats_q;
    reg [3:0]              rd_pop_beats_q;
    reg [3:0]              rd_prot_q;
    ...
    always @(posedge aclk or negedge aresetn) begin
        if (!aresetn) begin
            ...
            wr_beats_left_q <= 4'd0;
            wr_total_beats_q <= 4'd0;
            wr_done_beats_q <= 4'd0;
            wr_prot_q       <= 4'b0011;

            rd_active       <= 1'b0;
            rd_addr_q       <= {ADDR_WIDTH{1'b0}};
            rd_beats_left_q <= 4'd0;
            rd_total_beats_q <= 4'd0;
            rd_done_beats_q <= 4'd0;
            rd_pop_beats_q  <= 4'd0;
            rd_prot_q       <= 4'b0011;
            ...
            if (aw_take) begin
                wr_active        <= 1'b1;
                wr_addr_q        <= s_awaddr;
                wr_beats_left_q  <= s_awlen + 4'd1;
                wr_total_beats_q <= s_awlen + 4'd1;
                wr_done_beats_q  <= 4'd0;
                wr_prot_q        <= {1'b0, s_awcache[0], s_awprot[1], s_awprot[0]};
            end

            if (w_take) begin
                wr_addr_fifo[wr_wr_ptr] <= wr_addr_q;
                wr_prot_fifo[wr_wr_ptr] <= wr_prot_q;
                wr_data_fifo[wr_wr_ptr] <= s_wdata;
                wr_wr_ptr <= (wr_wr_ptr == WR_CMD_DEPTH-1) ? {WR_PTR_W{1'b0}} : (wr_wr_ptr + 1'b1);
                wr_addr_q <= wr_addr_q + {{(ADDR_WIDTH-2){1'b0}}, 2'b100};
                wr_beats_left_q <= wr_beats_left_q - 4'd1;
                if (wr_beats_left_q == 4'd1) begin
                    wr_active <= 1'b0;
                end
            end
            ...
            if (ar_take) begin
                rd_active        <= 1'b1;
                rd_addr_q        <= s_araddr;
                rd_beats_left_q  <= s_arlen + 4'd1;
                rd_total_beats_q <= s_arlen + 4'd1;
                rd_done_beats_q  <= 4'd0;
                rd_pop_beats_q   <= 4'd0;
                rd_prot_q        <= {1'b0, s_arcache[0], s_arprot[1], s_arprot[0]};
            end

            if (rd_active && !rd_full) begin
                rd_addr_fifo[rd_wr_ptr] <= rd_addr_q;
                rd_prot_fifo[rd_wr_ptr] <= rd_prot_q;
                rd_wr_ptr <= (rd_wr_ptr == RD_CMD_DEPTH-1) ? {RD_PTR_W{1'b0}} : (rd_wr_ptr + 1'b1);
                rd_addr_q <= rd_addr_q + {{(ADDR_WIDTH-2){1'b0}}, 2'b100};
                rd_beats_left_q <= rd_beats_left_q - 4'd1;
                if (rd_beats_left_q == 4'd1) begin
                    rd_active <= 1'b0;
                end
            end
            ...
            if (complete_write) begin
                wr_done_beats_q <= wr_done_beats_q + 4'd1;
                if ((wr_done_beats_q + 4'd1) == wr_total_beats_q) begin
                    b_pending <= 1'b1;
                end
            end
            ...
            if (complete_read) begin
                ...
                rd_done_beats_q <= rd_done_beats_q + 4'd1;
            end

            if (r_take) begin
                r_rd_ptr <= (r_rd_ptr == RESP_DEPTH-1) ? {RESP_PTR_W{1'b0}} : (r_rd_ptr + 1'b1);
                rd_pop_beats_q <= rd_pop_beats_q + 4'd1;
            end
            ...
            if (hready) begin
                ...
                if (issue_wr) begin
                    ...
                    pend_write <= 1'b0;
                    pend_rlast <= ((rd_done_beats_q + 4'd1) == rd_total_beats_q);
                end
            end
        end
    end
    ...
    wire _unused_meta = ^{
        hresp, s_awlen, s_awsize, s_awburst, s_awlock, s_wstrb, s_wlast,
        s_arlen, s_arsize, s_arburst, s_arlock
    };
